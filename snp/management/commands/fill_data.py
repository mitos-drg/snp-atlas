...
def create_annotations(snp):
    for i in range(randrange(5)):
        a = Annotation(snp=snp, text="Lorem ipsum dolor sit amet.", author=choice(User.objects.all()))
        a.save()
...
def create_snp(chromosome):
    ...
    s = SNP(chromosome=chromosome, position=randrange(1, 1000),
            ref_allele=allele, alt_allele=choice(alleles),
            maf=uniform(0.0, 0.5))
    s.save()
...
def create_animals():
    animals = [
        {"name": "Daniel", "latin": "Dama dama", "image": "img/daniel.jpg"},
        {"name": "Jeleń", "latin": "Cervus elaphus", "image": "img/jelen.jpg"},
        {"name": "Lew", "latin": "Panthera leo", "image": "img/lew.jpg"},
        {"name": "Pingwin", "latin": "Aptenodytes forsteri", "image": "img/pingwin.jpg"},
    ]
